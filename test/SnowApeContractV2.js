...
describe("SnowApeContractV2", function () {
    ...
    before(async () => {
        ...
        const SnowApeContract = await ethers.getContractFactory("SnowApeContractV2");
    ...
    it("Should win game", async function () {
        ...
        expect(await snowApeContractInstance.getBettingPoolBalance()).to.eql(BigNumber.from(5000));
        ...
        await snowApeContractInstance.determineOutputOfOnePlayerGame(addr1.address, 'the-game', true, 5000);

        expect(await snowApeContractInstance.getBalance()).to.eql(BigNumber.from(9500));
        expect(await snowApeContractInstance.getTotalPlayerValue()).to.eql(BigNumber.from(9500));
        expect(await snowApeContractInstance.getBettingPoolBalance()).to.eql(BigNumber.from(0));
        expect(await snowApeContractInstance.getUserBalance(owner.address)).to.eql(BigNumber.from(0));
        expect(await snowApeContractInstance.getUserBalance(addr1.address)).to.eql(BigNumber.from(9500));
    });
    ...
    it("Should lose game", async function () {
        ...
        expect(await snowApeContractInstance.getUserBalance(owner.address)).to.eql(BigNumber.from(0));
        expect(await snowApeContractInstance.getUserBalance(addr1.address)).to.eql(BigNumber.from(5000));

        await snowApeContractInstance.connect(addr1).initializeOnePlayerGame(4500, 'the-game');

        expect(await snowApeContractInstance.getBalance()).to.eql(BigNumber.from(4500));
        expect(await snowApeContractInstance.getTotalPlayerValue()).to.eql(BigNumber.from(0));
        expect(await snowApeContractInstance.getBettingPoolBalance()).to.eql(BigNumber.from(4500));
        expect(await snowApeContractInstance.getUserBalance(owner.address)).to.eql(BigNumber.from(0));
        expect(await snowApeContractInstance.getUserBalance(addr1.address)).to.eql(BigNumber.from(0));

        await snowApeContractInstance.determineOutputOfOnePlayerGame(addr1.address, 'the-game', false, 0);
